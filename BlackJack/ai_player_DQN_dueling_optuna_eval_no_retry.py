...
def select_action_no_retry(state, strategy, agent):
    """Select action but disallow Action.RETRY in all cases."""
    retry_idx = orig.action_to_index(orig.Action.RETRY)

    # QMAX
    if strategy == orig.Strategy.QMAX:
        ...
        mask = orig.get_valid_action_mask_from_state(state)
        # force-disable RETRY
        mask[retry_idx] = False
        q_vals_masked = np.copy(q_vals)
        q_vals_masked[~mask] = -1e9
        return orig.index_to_action(int(np.argmax(q_vals_masked)))

    # E_GREEDY
    elif strategy == orig.Strategy.E_GREEDY:
        if np.random.rand() < agent.epsilon:
            mask = orig.get_valid_action_mask_from_state(state)
            mask[retry_idx] = False
            valid_indices = np.where(mask)[0]
            chosen = np.random.choice(valid_indices)
            return orig.index_to_action(int(chosen))
        else:
            state_tensor = torch.tensor(state, dtype=torch.float32, device=agent.device).unsqueeze(0)
            q_vals = agent.original_q_network(state_tensor).detach().cpu().numpy()[0]
            mask = orig.get_valid_action_mask_from_state(state)
            mask[retry_idx] = False
            q_vals[~mask] = -1e9
            return orig.index_to_action(int(np.argmax(q_vals)))

    # Random strategy (fallback)
    else:
        mask = orig.get_valid_action_mask_from_state(state)
        mask[retry_idx] = False
        valid_indices = np.where(mask)[0]
...
def train_episode(agent: orig.Agent, n_games: int, args, early_stopping=None):
    ...
    for n in tqdm(range(1, n_games)):
        ...
        while True:
            if args.testmode:
                action = select_action_no_retry(state, orig.Strategy.QMAX, agent)
            else:
                action = select_action_no_retry(state, orig.Strategy.E_GREEDY, agent)
            # No RETRY allowed: select_action_no_retry already enforces it

            reward, done, status = orig.act(action)
            if action == orig.Action.RETRY:
                # safety: ensure retry counter is not incremented if somehow chosen
...
def evaluate_agent_money(agent: orig.Agent, n_games: int, verbose=False):
    ...
    for n in range(1, n_games+1):
        ...
        while True:
            action = select_action_no_retry(state, orig.Strategy.QMAX, agent)
            # No RETRY allowed in eval as well
            reward, done, status = orig.act(action)
            if action == orig.Action.RETRY:
                orig.g_retry_counter += 1
...
def main():
    parser = argparse.ArgumentParser(description='Dueling DQN Optuna evaluator (train then evaluate average money) - NO RETRY')
    ...
    parser.add_argument('--save', type=str, default='dqn_model_optuna_best_no_retry.pth', help='save path for best model')
    parser.add_argument('--history', type=str, default=os.path.join('play_log_optuna_eval_no_retry.csv'), help='CSV path to save per-game summaries for plotting')
    ...
    study = optuna.create_study(direction='maximize', study_name='dqn_dueling_eval_optuna_no_retry')
