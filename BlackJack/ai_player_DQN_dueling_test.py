...
LEARNING_RATE = 0.01 # 学習率（DQN用に小さめ）
...
class DuelingQNetwork(torch.nn.Module):
    """Dueling DQN: simplified shared body and separate value/advantage streams.

    This matches the original (pre-BatchNorm/Dropout) architecture:
      - two shared layers (hid=256)
      - value stream: hid -> 128 -> 1
      - advantage stream: hid -> 128 -> action_size
    The combine rule is Q = V + (A - mean(A)).
    """
...
class Agent:
    ...
    def get_best_action(self, state: np.ndarray) -> int:
        # state is expected to be a flattened numpy array (as produced by get_state)
        state_np = state if isinstance(state, np.ndarray) else np.array(state, dtype=np.float32)

        state_tensor = torch.tensor(state_np, dtype=torch.float32, device=self.device).unsqueeze(0)
        self.original_q_network.eval()  # Set model to evaluation mode
        with torch.no_grad():
            q_vals = self.original_q_network(state_tensor).detach().cpu().numpy()[0]
        self.original_q_network.train()  # Restore training mode

        # Apply valid-action mask (same logic as training code)
        try:
            mask = get_valid_action_mask_from_state(state_np)
            q_masked = np.copy(q_vals)
            q_masked[~mask] = -1e9
            return int(np.argmax(q_masked))
        except Exception:
            # If masking fails for any reason, fallback to raw argmax
            ...
            if not args.testmode:
                try:
                    ...
                    if action == Action.DOUBLE_DOWN:
                        ...
                        if 9 <= prev_score <= 11:
                            total_reward += 0.1  # 良いタイミングでのDD
                    
                    # 6. 終了時の勝敗に基づく大きな報酬
                    if done:
                        if reward > 0:
                            total_reward += 1.0  # 勝利ボーナス
                        elif reward < 0:
                            total_reward -= 0.5  # 敗北ペナルティ
